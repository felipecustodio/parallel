...
import sys
sys.stdout = open('test_output_par.out', 'w')
...
grades_2d = genfromtxt('matriz_gerada_par.csv', delimiter=',')
...
for index_region, region in enumerate(grades):
    ...
    for index_city, city in enumerate(region):
        ...
        print(f"menor: {menor:.2f}, maior: {maior:.2f}, mediana: {mediana:.2f}, media: {media:.2f} e DP: {DP:.2f}")
